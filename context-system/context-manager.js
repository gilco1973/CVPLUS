...
class CVPlusContextManager {
    ...
    async initialize() {
        ...
        // Populate tiers
        await this.populateContextTiers();
    ...
    async loadContextIndexes() {
        ...
        for (const tier of ['tier1_critical', 'tier2_contextual', 'tier3_archive']) {
            ...
            try {
                ...
                const tierKey = tier.split('_')[0]; // tier1 -> tier1, becomes 'critical'
    ...
    async populateContextTiers() {
        ...
        const noiseReduction = (filteredFiles / totalFiles) * 100;
    ...
    async shouldIncludeInActiveContext(metadata, tier) {
        ...
        // Content filtering
        if (metadata.contentPreview.toLowerCase().includes('todo') || 
    ...
    async getSystemStatus() {
        const status = {
            ...
            memoryUsage: await this.memoryManager.getMemoryUsageReport(),
...
if (require.main === module) {
    ...
    async function runDemo() {
        ...
        // Show system status
        console.log(`\n${'='.repeat(60)}`);
        const status = await contextManager.getSystemStatus();
