  ...
  async (request) => {
    ...
    try {
      ...
      if (!jobId || typeof jobId !== 'string') {
        logger.logWarning('BookMeeting: Invalid jobId provided', {
      ...
      if (!duration || typeof duration !== 'number' || duration < 15 || duration > 180) {
        logger.logWarning('BookMeeting: Invalid duration provided', {
          event: 'meeting.book.validation_failed',
          field: 'duration',
          value: duration,
      ...
      if (!attendeeEmail || typeof attendeeEmail !== 'string' || !attendeeEmail.includes('@')) {
        logger.logWarning('BookMeeting: Invalid attendee email provided', {
          event: 'meeting.book.validation_failed',
          field: 'attendeeEmail',
      ...
      if (meetingType && typeof meetingType !== 'string') {
        logger.logWarning('BookMeeting: Invalid meeting type provided', {
          event: 'meeting.book.validation_failed',
          field: 'meetingType',
      ...
      logger.logDebug('BookMeeting: Input validation passed', {
      ...
      logger.logDebug('BookMeeting: Fetching job document', {
      ...
      if (!jobDoc.exists) {
        logger.logWarning('BookMeeting: Job not found', {
      ...
      if (!jobData?.parsedData) {
        logger.logError('BookMeeting: CV data missing from job', new Error('CV data not found'), {
      ...
      if (jobData.userId !== user.uid) {
        logger.logError('BookMeeting: Unauthorized access attempt', new Error('Unauthorized access'), {
      ...
      logger.logDebug('BookMeeting: Job ownership verified', {
      ...
      logger.logDebug('BookMeeting: Extracted professional info', {
      ...
      logger.logInfo('BookMeeting: Creating calendar meeting invite', {
      ...
      logger.logInfo('BookMeeting: Calendar meeting invite created successfully', {
      ...
      logger.logDebug('BookMeeting: Storing meeting request in Firestore', {
      ...
      logger.logError('BookMeeting: Function execution failed', error as Error, {
