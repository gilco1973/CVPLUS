...
import { UserSubscriptionData } from '../cached-subscription.service';
...
class SubscriptionCacheService {
   ...
   */
  async hasFeature(userId: string, featureKey: keyof UserSubscriptionData['features']): Promise<{
    hasFeature: boolean;
    cached: boolean;
    responseTime: number;
  }> {
  ...
  async hasFeatures(
    userId: string, 
    featureKeys: (keyof UserSubscriptionData['features'])[]
  ): Promise<{
  ...
  private normalizeSubscriptionData(data: any): UserSubscriptionData {
    const normalized: UserSubscriptionData = {
      ...
      usage: {
        cvGenerations: data.usage?.cvGenerations || 0,
        apiCalls: data.usage?.apiCalls || 0,
        storageUsed: data.usage?.storageUsed || 0,
        videosGenerated: data.usage?.videosGenerated || 0,
        podcastsCreated: data.usage?.podcastsCreated || 0
      },
      limits: {
        cvGenerations: data.limits?.cvGenerations || (data.lifetimeAccess ? -1 : 3),
        apiCalls: data.limits?.apiCalls || (data.lifetimeAccess ? -1 : 100),
        storageLimit: data.limits?.storageLimit || (data.lifetimeAccess ? -1 : 100 * 1024 * 1024),
        videosPerMonth: data.limits?.videosPerMonth || (data.lifetimeAccess ? -1 : 0),
        podcastsPerMonth: data.limits?.podcastsPerMonth || (data.lifetimeAccess ? -1 : 0)
      },
      metadata: {
