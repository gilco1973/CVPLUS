...
class FeatureAccessCacheService {
    ...
    try {
      ...
      const batchResult = await cacheService.getBatch<{
        ...
        async (missedKeys) => {
          ...
          const subscriptionResult = await subscriptionCacheService.getUserSubscription(userId);
  ...
  private async validateFeatureAccess(request: FeatureAccessRequest): Promise<{
    ...
    try {
      ...
      const subscriptionResult = await subscriptionCacheService.getUserSubscription(request.userId);
      const subscription = subscriptionResult.subscription;
