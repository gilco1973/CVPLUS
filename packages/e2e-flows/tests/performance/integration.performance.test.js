...
describe('Integration Performance Tests', () => {
    ...
    beforeAll((done) => {
        // Initialize services
        mockDataService = new MockDataService_1.MockDataService({
            maxSize: 500 * 1024 * 1024, // 500MB
            maxAge: 60 * 60 * 1000, // 1 hour
            compression: false,
            encryption: false
        });
        apiTestingService = new APITestingService_1.APITestingService();
        e2eFlowsService = new E2EFlowsService_1.E2EFlowsService();
        performanceRunner = new performance_utilities_1.PerformanceTestRunner(50); // 50ms sampling
    ...
    describe('End-to-End Workflow Performance', () => {
        test('should complete full CVPlus workflow simulation within performance targets', async () => {
            const testResult = await performanceRunner.runTest('CVPlus E2E Workflow Simulation', async (monitor) => {
                ...
                const apiResults = await apiTestingService.executeMultipleTestCases(testCases.map(tc => tc.id), {
                    baseUrl,
                    parallel: true,
                    maxConcurrency: 25,
                    timeout: 10000,
        ...
        test('should handle concurrent workflows without resource contention', async () => {
            ...
            performance_utilities_1.performanceAssertions.expectDurationUnder(duration, 300000, // 5 minutes for concurrent workflows
            'Concurrent workflows completion time');
            performance_utilities_1.performanceAssertions.expectMemoryUnder(memoryUsed, 1024 * 1024 * 1024, // 1GB
    ...
    describe('Service Integration Load Testing', () => {
        test('should maintain performance under integrated high load', async () => {
            const monitor = new performance_utilities_1.PerformanceMonitor(100);
            monitor.start();
            try {
                ...
                performance_utilities_1.performanceAssertions.expectDurationUnder(metrics.duration, 600000, // 10 minutes
                'Integrated high load test duration');
                performance_utilities_1.performanceAssertions.expectMemoryUnder(metrics.memoryUsed, 1024 * 1024 * 1024, // 1GB
    ...
    describe('Resource Efficiency Testing', () => {
        test('should demonstrate efficient resource utilization across services', async () => {
            const efficiency = await performance_utilities_1.PerformanceUtils.measureMemoryUsage(async () => {
                const tasks = [];
            ...
            performance_utilities_1.performanceAssertions.expectMemoryUnder(efficiency.memoryUsed, 300 * 1024 * 1024, // 300MB for all operations
    ...
    const runSingleWorkflow = async (workflowId, seed) => {
        try {
            // Generate data
            ...
            const apiResult = await apiTestingService.executeTestCase(testCase, `http://localhost:${mockServerPort}`);
...
//# sourceMappingURL=integration.performance.test.js.map